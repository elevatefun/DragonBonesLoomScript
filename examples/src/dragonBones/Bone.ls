package dragonBones {
    ...
    public class Bone extends DBObject {
        ...
        override public function set visible(value:Boolean):void
        {
            ...
            {
                this._visible = value;
                var i:int = int(_children.length);
                var child:DBObject;
                while(i --)
                {
                    ...
                    if(child is Slot){
                        ...
                        Slot(child).updateVisible(this._visible);
        ...
        public function removeChild(child:DBObject):void
        {
            ...
            var index:int = int(_children.indexOf(child));
            if (index >= 0)
            {
                //_children.fixed = false;
        ...
        public function arriveAtFrame(frame:Frame, timelineState:TimelineState, animationState:AnimationState, isCross:Boolean):void
        {
            if(frame)
            {
                var mixingType:int = int(animationState.getMixingTransform(name));
                if(animationState.displayControl && (mixingType == 2 || mixingType == -1))
                {
                    ...
                    )
                    {
                        ...
                        if(_slot)
                        {
                            var displayIndex:int = int(tansformFrame.displayIndex);
                            if(displayIndex >= 0)
                            {
                ...
                if(frame.action)
                {
                    ...
                    for each(var child:DBObject in _children){
                        ...
                        if(child is Slot){
                            ...
                            var childArmature:Armature = Slot(child).childArmature;
                            if(childArmature){
