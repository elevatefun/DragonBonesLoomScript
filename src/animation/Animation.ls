    ...
    public class Animation
    {
        ...
        public function get isComplete():Boolean
        {
            if(_lastAnimationState)
            {
                ...
                var j:int = int(_animationLayer.length);
                while(j --)
                {
                    var animationStateList:Vector.<AnimationState> = _animationLayer[j];
                    var i:int = int(animationStateList.length);
                    while(i --)
                    {
        ...
        public function dispose():void
        {
            ...
            stop();
            var i:int = int(_animationLayer.length);
            while(i --)
            {
                var animationStateList:Vector.<AnimationState> = _animationLayer[i];
                var j:int = int(animationStateList.length);
                while(j --)
                {
        ...
        {
            var l:int = int(_animationLayer.length);
            if(l == 0)
            {
                return null;
            }
            ...
            while(k --)
            {
                ...
                while(i --)
                {
                    ...
                    for(j=0; j<stateListLength; j++) {
                        ...
                        {
                            weight = animationState._fadeWeight * animationState.weight * weightLeft;
        ...
        private function addLayer(layer:uint):uint
        {
            if(layer >= _animationLayer.length)
            {
                ...
                _animationLayer.push(new Vector.<AnimationState>());
            }
            return layer;
        }
